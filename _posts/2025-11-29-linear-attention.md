...
The denominator serves primarily for numerical stability. Ignoring the causal mask, the core mechanism of attention is really $O=\exp(QK^\top)V$. The problem of softmax attention is that to compute $\exp(QK^\top)$, both space and time complexity are $O(n^2)$.
...
If we take $\phi$ to be the identity mapping, we get $O = Q(K^\top V)$. Since $K^\top V$ is a $d \times d$ matrix, this reformulation reduces complexity from $O(n^2)$ to $O(n)$.  
...
This is our vanilla linear attention. It's clear that the causal form of attention can be written as a linear RNN with $S_t$ as the hidden state. Every step's time complexity is constant, so the total time complexity is $O(n)$.

## Linear Attention As Test Time Training

![TTT](/img/TTT.png)

There's a beautiful conceptualization of linear attention as Test Time Training (TTT). TTT can be used to construct RNNs. Here's the procedure: Let the current model's parameter be $S_{t-1}$. The optimizer (SGD) receives new data $k_t, v_t$. The optimizer updates the parameter to $S_t$ using the new data and returns the prediction result $f(S_{t-1}; q_t)$. So all RNNs created with TTT can be written as:
...
Let's calculate the gradient of $\mathcal{L}$ with respect to $S$:
...
Plugging this back into the TTT update rule, we get:
...
The gradient of this loss function with respect to $y$ is $y-v$. Plugging this back to (3), we get:
...
Plugging this back in the TTT update rule and keeping the learning rate time dependent, we get:
...
If we set the loss function to be 

$$\frac{1}{2} ||y-v||^2 + \frac{1-\gamma}{\eta}||S||_F^2$$
...
We need not restrict ourselves to just using different loss functions. We can also experiment with different update rules. For example, in the Titans paper, the authors add a momentum term to the SGD update rule. The Test-Time Training Done Right paper has explored the possibility of using Muon optimizer instead of SGD. 
